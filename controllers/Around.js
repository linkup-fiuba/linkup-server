	...
	this.config.async.waterfall([
	    function (callback) {
	    	...
	    	config.redisLib.getHash(config.usersKey+userId, function (err, user) {
	    		...
	    		config.redisLib.getHash(config.preferencesKey+userId, function (err, userPreferences) {
					if (err) return callbackAround(err, null);
	    ...
	    function (user, userPreferences, callback) {
	    	...
	    	this.config.redisLib.getFromSet(this.config.genderKey+userPreferences.gender, function (err, usersIds) {
	    ...
	    function (user, userPreferences, usersIds, callback) {
		    this.config.async.each(usersIds, function (id, callbackIt) {
				if (userId == id) {
					return callbackIt();
				}
				this.config.async.waterfall([
					function (cb) {
						this.config.redisLib.getHash(this.config.usersKey+id, function (err, otherUser) {
							if (err) return cb(err, null);
				    ...
				    function (user, otherUser, cb) {
				    	...
				    	this.config.redisLib.getHashField(this.config.preferencesKey+id, 'gender', function (err, userPref) {
				    		...
				    		validatePossibleAround(user.gender, userPreferences.gender, otherUser.gender, userPref, function (validate) {
				    			if (validate) {
				    				...
				    				this.config.redisLib.addToSet(this.config.aroundKey+userId, id, function (err, reply) {
				    					if (err) return cb(err, null);
										this.config.redisLib.addToSet(this.config.aroundKey+id, userId, function(err, reply) {
											if (err) return cb(err, null);
				    ...
				    function (otherUser, cb) {
						...
						//save info of around 
						this.config.redisLib.setHash(this.config.aroundKey+userId+':'+id, userModel, function (err, responseSave) {
							if (err) return cb(err, null);
							this.config.redisLib.setHash(this.config.aroundKey+id+':'+userId, actualUserModel, function (err, responseSave) {
								if (err) return cb(err, null);
								return callbackIt();
							})
						})
				...
				config.async.waterfall([
				    ...
				    function (user, otherUser, cb) {
						...
						config.redisLib.setHash(config.aroundKey+userId+':'+userMatch.id, userModel, function (err, responseSave) {
							if (err) return cb(err, null);
							return callbackIt();
							/*this.config.redisLib.setHash(this.config.aroundKey+userMatch.id+':'+userId, actualUserModel, function (err, responseSave) {
								if (err) return cb(err, null);
								return callbackIt();
							})*/
						})
...
function deleteAroundUser(userId, userIdRemove, callbackDelete) {
	...
	config.async.waterfall([
	    ...
	    function (callback) {
	    	config.redisLib.removeFromSet(config.aroundKey+userIdRemove, userId, function (err, replySetTwo) {
				if (err) return callback(err, null);
				return callback(null);
			})
	    },
	    function (callback) {
	    	config.redisLib.deleteKey(config.aroundKey+userId+':'+userIdRemove, function (err, replyRemOne) {
				if (err) return callback(err, null);
				config.redisLib.deleteKey(config.aroundKey+userIdRemove+':'+userId, function (err, replyRemTwo) {
					if (err) return callback(err, null);
					return callback(null);
				})		
			})
